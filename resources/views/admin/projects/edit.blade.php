    ...
    <div class="flex items-center justify-between">
        <div>
            <div class="flex items-center gap-2 text-xs font-semibold text-slate-400 uppercase tracking-wider">
                ...
                <span>/</span>
                <span class="text-indigo-400">Edit Project</span>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div>
                    <label for="slug" class="block text-xs font-semibold text-slate-300 uppercase tracking-wider mb-2">
                        URL Slug <span class="text-indigo-400">*</span>
                    </label>
                ...
                <div class="sm:col-span-2">
                    ...
                    <textarea name="description" id="description" rows="4" required
                        class="w-full px-4 py-3 bg-slate-950/60 border border-slate-800 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 rounded-xl text-sm text-slate-100 placeholder-slate-600 outline-none transition-all leading-relaxed">{{ old('description', $project->description) }}</textarea>
                    @error('description')
                        <p class="text-xs text-red-400 mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="sm:col-span-2">
                    ...
                    <input type="text" name="tech_stack" id="tech_stack"
                        ...
                        placeholder="e.g. Laravel 11, Tailwind CSS, MySQL"
                        class="w-full px-4 py-3 bg-slate-950/60 border border-slate-800 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 rounded-xl text-sm text-slate-100 placeholder-slate-600 outline-none transition-all">
                    <p class="text-xs text-slate-500 mt-1.5">Enter technology tags separated by commas.</p>
                    @error('tech_stack')
                        <p class="text-xs text-red-400 mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="url" name="demo_url" id="demo_url" value="{{ old('demo_url', $project->demo_url) }}" placeholder="https://example.com/demo"
                        class="w-full px-4 py-3 bg-slate-950/60 border border-slate-800 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 rounded-xl text-sm text-slate-100 placeholder-slate-600 outline-none transition-all">
                    @error('demo_url')
                        <p class="text-xs text-red-400 mt-1">{{ $message }}</p>
                    @enderror
